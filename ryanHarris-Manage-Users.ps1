  ...
  # Get-ADOrganizationalUnit -Filter 'Name -like "*"' | ft 
    ...
    #$group = $group.Replace(" ","");
      try{
      New-ADGroup $group -GroupScope Global -Path "OU=$ou, $domain";
      write-host("Creating $group organizational Unit");
       }
      catch{
      $groupExisted = Get-ADGroup -Identity "$group";
      write-host("The Group ($group) exists on the Domain in the Organizational Unit ($ou)"); 
...
#Make Necessary Users to Create
 ...
 foreach($user in $Users){
     #checks to see if we must make user
         try{
         New-ADUser -Name $($user.account) -GivenName "$($user.firstname)" -Surname "$($user.lastname)"  -Path "OU=$ou,$domain" -AccountPassword (ConvertTo-SecureString -AsPlainText $user.password -force) -Enabled $true;
         write-host "creating user $($user.account)";          
        }
         catch {
         #property of an object in the expanision string it needs alittle help
         $userFound = Get-ADUser -Filter * -SearchBase "OU=$ou, $domain"| Select-Object $($user.account)
         write-host "User $($user.account) found in $ou"
         }
         ...
         #checks to see if we need to add use to group
          foreach($group in $user.memberof.group){
             ...
             try{
                 ...
                 Add-ADGroupMember -Identity "CN=$group, OU=$ou, $domain" -Members $($user.account);
